...
import pickle
# rom dataset.utils import get_imitation_data
...
def call_main(details):
    ...
    if 'LoRALearner' in details['rl_config']['model_cls'] and details['com_method'] == 0:
        print(details['env_name'])
        #env = gym.make(details['env_name'])
        env = gym.make('Meta-World/MT1', env_name=details['env_name'], seed=seed)
    ...
    if 'LoRA' in model_cls:
        details['pretrain_model'] = f"pretrained/model1000000.pickle"
    ...
    for i in tqdm(range(60000), smoothing=0.1):
        ...
        if i % details['eval_interval'] == 0 or i == 0:
            ...
            for inference_params in details['inference_variants']:
                ...
                if model_cls == 'LoRASLearner':
                    ...
                    print(f"reward_eval_info: {reward_eval_info}")
                    ...
                    print(f"Q Normalized return: {reward_eval_info['q_normalized_return']}, Q Normalized cost: {reward_eval_info['q_normalized_cost']}")
                    ...
                    print(f"cost_eval_info: {cost_eval_info}")
                    ...
                    print(f"QC Normalized return: {cost_eval_info['qc_normalized_return']}, QC Normalized cost: {cost_eval_info['qc_normalized_cost']}")
                    ...
                    print(f"eval_info (score_com): {eval_info}")
                    eval_info['normalized_return'], eval_info['normalized_cost'] = env.get_normalized_score(eval_info['return'], eval_info['cost'])
                    print(f"Normalized return: {eval_info['normalized_return']}, Normalized cost: {eval_info['normalized_cost']}")
                    wandb.log({f"eval_bc/{inference_params}_{k}": v for k, v in eval_info.items()}, step=i)
                    print("checkpoint10")
                    ...
                    print(f"eval_info (action_com): {eval_info}")
                    eval_info['normalized_return'], eval_info['normalized_cost'] = env.get_normalized_score(eval_info['return'], eval_info['cost'])
                    print(f"Normalized return: {eval_info['normalized_return']}, Normalized cost: {eval_info['normalized_cost']}")
                    wandb.log({f"eval_bc/{inference_params}_{k}": v for k, v in eval_info.items()}, step=i)
